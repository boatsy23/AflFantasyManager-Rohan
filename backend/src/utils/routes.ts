...
import { registerFantasyRoutes } from "../routes/fantasy-routes";
import roleApi from "../routes/role-api";
import captainApi from "../routes/captain-api";
import priceApi from "../routes/price-api";
import fixtureApi from "../routes/fixture-api";
import contextApi from "../routes/context-api";
import teamApi from "../routes/team-api";
...
// import { roundDataProcessor } from './round-data-processor'; // File removed
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/players/round/:round", async (req, res) => {
    try {
      ...
      const masterStatsMap = new Map<string, MasterStats>(
        masterStatsData.players.map((p: any) => [p.name, p])
      );
  ...
  app.get("/api/team/fantasy-data", async (req, res) => {
    try {
      const filePath = path.join(process.cwd(), 'data/team/afl_fantasy_team.json');
  ...
  app.get("/api/team/lineup", async (req, res) => {
    try {
      const filePath = path.join(process.cwd(), 'data/team/afl_fantasy_team.json');
      console.log('[lineup] Resolved file path:', filePath);
      console.log('[lineup] File exists:', fs.existsSync(filePath));
      
      if (!fs.existsSync(filePath)) {
        console.error('[lineup] File not found at:', filePath);
        return res.status(404).json({
          status: 'error',
      ...
      console.log('[lineup] Successfully found team data file');
  ...
  // AFL Fantasy API integration routes - DISABLED (API removed)
  // app.get("/api/afl-fantasy/test", async (req, res) => {
  //   // Route disabled - AFL Fantasy API removed
  // });
  ...
  // AFL Fantasy scraping endpoint (disabled - service removed)
