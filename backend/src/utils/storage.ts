import {
  ...
  type DfsPlayer, type InsertDfsPlayer,
  dfsPlayers,
  players
} from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  // DFS Player methods
  getAllDfsPlayers(): Promise<DfsPlayer[]>;
  getDfsPlayer(id: number): Promise<DfsPlayer | undefined>;
  getDfsPlayerByDfsId(dfsPlayerId: string): Promise<DfsPlayer | undefined>;
  getDfsPlayersByTeam(team: string): Promise<DfsPlayer[]>;
  getDfsPlayersByPosition(position: string): Promise<DfsPlayer[]>;
  searchDfsPlayers(query: string): Promise<DfsPlayer[]>;
  createDfsPlayer(player: InsertDfsPlayer): Promise<DfsPlayer>;
  updateDfsPlayer(id: number, player: Partial<InsertDfsPlayer>): Promise<DfsPlayer | undefined>;
  upsertDfsPlayer(player: InsertDfsPlayer): Promise<DfsPlayer>;
  ...
  clearRoundPerformances(teamId: number): Promise<void>;
}
...
export class MemStorage implements IStorage {
  ...
  // Note: dfsPlayers are stored in PostgreSQL, not in-memory
  ...
  private initializeData() {
    ...
    // Sample player creation disabled - we now have real players from DFS data
    // The unique constraint on (name, team) prevents duplicate creation
      ...
      this.createTeam({
        ...
        createOpponentTeams().then(opponentTeams => {
          ...
          Promise.all(createLeaguesPromises).then(() => {
            // Demo performance data removed - use real historical data instead
            // Historical data can be loaded via POST /api/teams/:teamId/performances/load-historical
          });
        });
      });
    });
  ...
  // Player Methods - Use database for persistence
  async getAllPlayers(): Promise<Player[]> {
    return await db.select().from(players).orderBy(desc(players.averagePoints));
  }
  ...
  async getPlayersByPosition(position: string): Promise<Player[]> {
    return await db.select().from(players)
      ...
      .orderBy(desc(players.averagePoints));
  }
  ...
  async createTeam(insertTeam: InsertTeam): Promise<Team> {
    ...
    const team: Team = { 
      ...insertTeam, 
      id,
      value: insertTeam.value ?? 10000000,
      score: insertTeam.score ?? 0,
      captainId: insertTeam.captainId ?? null,
      overallRank: insertTeam.overallRank ?? null,
      trades: insertTeam.trades ?? 2
    };
  ...
  async addPlayerToTeam(insertTeamPlayer: InsertTeamPlayer): Promise<TeamPlayer> {
    ...
    const teamPlayer: TeamPlayer = { 
      ...insertTeamPlayer, 
      id,
      isOnField: insertTeamPlayer.isOnField ?? false
    };
  ...
  async addTeamToLeague(insertLeagueTeam: InsertLeagueTeam): Promise<LeagueTeam> {
    ...
    const leagueTeam: LeagueTeam = { 
      ...insertLeagueTeam, 
      id,
      wins: insertLeagueTeam.wins ?? 0,
      losses: insertLeagueTeam.losses ?? 0,
      pointsFor: insertLeagueTeam.pointsFor ?? 0
    };
  ...
  async createMatchup(insertMatchup: InsertMatchup): Promise<Matchup> {
    ...
    const matchup: Matchup = { 
      ...insertMatchup, 
      id,
      team1Score: insertMatchup.team1Score ?? null,
      team2Score: insertMatchup.team2Score ?? null
    };
  ...
  async createRoundPerformance(insertPerf: InsertRoundPerformance): Promise<RoundPerformance> {
    ...
    const perf: RoundPerformance = { 
      ...insertPerf, 
      id,
      projectedScore: insertPerf.projectedScore ?? null,
      rank: insertPerf.rank ?? null
    };
  ...
  async clearRoundPerformances(teamId: number): Promise<void> {
    const perfIds = Array.from(this.roundPerformances.entries())
      .filter(([_, perf]) => perf.teamId === teamId)
      .map(([id]) => id);
    
    perfIds.forEach(id => this.roundPerformances.delete(id));
  }

  // DFS Player Methods - Use database for persistence
  async getAllDfsPlayers(): Promise<DfsPlayer[]> {
    return await db.select().from(dfsPlayers);
  }

  async getDfsPlayer(id: number): Promise<DfsPlayer | undefined> {
    const results = await db.select().from(dfsPlayers).where(eq(dfsPlayers.id, id));
    return results[0];
  }

  async getDfsPlayerByDfsId(dfsPlayerId: string): Promise<DfsPlayer | undefined> {
    const results = await db.select().from(dfsPlayers).where(eq(dfsPlayers.dfsPlayerId, dfsPlayerId));
    return results[0];
  }

  async getDfsPlayersByTeam(team: string): Promise<DfsPlayer[]> {
    return await db.select().from(dfsPlayers).where(eq(dfsPlayers.team, team));
  }

  async getDfsPlayersByPosition(position: string): Promise<DfsPlayer[]> {
    return await db.select().from(dfsPlayers).where(eq(dfsPlayers.position, position));
  }

  async searchDfsPlayers(query: string): Promise<DfsPlayer[]> {
    return await db.select().from(dfsPlayers).where(
      or(
        ilike(dfsPlayers.name, `%${query}%`),
        ilike(dfsPlayers.team, `%${query}%`),
        ilike(dfsPlayers.position, `%${query}%`)
      )
    );
  }

  async createDfsPlayer(insertPlayer: InsertDfsPlayer): Promise<DfsPlayer> {
    const result = await db.insert(dfsPlayers).values(insertPlayer).returning();
    return result[0];
  }

  async updateDfsPlayer(id: number, playerData: Partial<InsertDfsPlayer>): Promise<DfsPlayer | undefined> {
    const result = await db.update(dfsPlayers)
      .set(playerData)
      .where(eq(dfsPlayers.id, id))
      .returning();
    return result[0];
  }

  async upsertDfsPlayer(insertPlayer: InsertDfsPlayer): Promise<DfsPlayer> {
    // Check if player with this dfsPlayerId already exists
    const existing = await this.getDfsPlayerByDfsId(insertPlayer.dfsPlayerId);
    
    if (existing) {
      // Update existing player
      const updated = await this.updateDfsPlayer(existing.id, insertPlayer);
      return updated!;
    } else {
      // Create new player
      return await this.createDfsPlayer(insertPlayer);
    }
  }
}
