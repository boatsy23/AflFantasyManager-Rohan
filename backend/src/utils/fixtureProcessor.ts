...
export class FixtureProcessor {
  ...
  private loadFixtures() {
    try {
      // Try multiple possible fixture file locations
      const possiblePaths = [
        path.join(process.cwd(), 'attached_assets/afl_fixture_2025.json'),
        path.join(process.cwd(), 'attached_assets/afl_fixture_2025_1753111987231.json'),
        path.join(process.cwd(), 'attached_assets/afl_fixture_1760682396719.json'),
        path.join(process.cwd(), 'server/data/afl_fixture_2025.json'),
        path.join(process.cwd(), 'data/afl_fixture.json')
      ];
      ...
      for (const fixturePath of possiblePaths) {
        if (fs.existsSync(fixturePath)) {
          ...
          console.log(`Loaded ${this.fixtures.length} fixture matches from ${fixturePath}`);
          loaded = true;
          break;
        }
      }
      ...
      if (!loaded) {
        console.log('No fixture file found - continuing with empty fixtures');
        this.fixtures = [];
      }
    } catch (error) {
      console.log('Error loading fixture data - continuing with empty fixtures');
      this.fixtures = [];
    }
  }
  ...
  public getLastFixtures(team: string, count: number = 5): TeamFixture[] {
    const teamFixtures = this.teamFixtures.get(team);
    if (!teamFixtures) return [];

    // Get the last N fixtures (most recent games)
    return teamFixtures
      .sort((a, b) => b.round - a.round) // Sort descending by round
      .slice(0, count)
