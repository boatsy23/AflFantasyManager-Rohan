...
// import { aflFantasyIntegration } from '../afl-fantasy-integration'; // File removed
import { MasterDataService } from '../services/MasterDataService';
import fs from 'fs';
import path from 'path';

// Stub replacement since afl-fantasy-integration was removed
interface AFLPlayer {
  name: string;
  price: number;
...
const aflFantasyIntegration = {
  isAuthenticated: () => false,
  fetchUserTeam: async () => null,
  fetchPlayerData: async (): Promise<AFLPlayer[]> => [],
  fetchUserPerformance: async () => null,
  fetchFixtures: async () => []
};
...
router.get('/team/integrated', async (req, res) => {
  try {
    ...
    try {
      const teamData = JSON.parse(fs.readFileSync('./data/team/afl_fantasy_team.json', 'utf8'));
      return res.json({
        source: 'local_data',
        authenticated: false,
        data: teamData,
...
router.get('/players/integrated', async (req, res) => {
  try {
    ...
    const localData = await MasterDataService.getAllPlayers();
      ...
      const mergedData = localData.map(localPlayer => {
        ...
        if (aflPlayer) {
          return {
            ...
            averagePoints: aflPlayer.averagePoints || localPlayer.average_points,
            lastScore: aflPlayer.lastScore || localPlayer.last_score,
            breakeven: aflPlayer.breakeven || localPlayer.break_even,
...
router.get('/performance/integrated', async (req, res) => {
  try {
    ...
    // Return empty structure indicating authentication needed
    ...
    try {
      ...
      positions.forEach(pos => {
        if (teamData[pos]) {
          ...
          teamData[pos].forEach((player: any) => teamValue += player.price || 0);
        }
      });
