...
// import { getPlayerByName, normalizeName as normalizePlayerName } from './footywire-integration'; // File removed

// Stub replacements since footywire-integration was removed
const getPlayerByName = async (name: string) => null;
const normalizePlayerName = (name: string) => name.toLowerCase().trim();
...
teamApi.post("/upload", async (req, res) => {
  ...
  try {
    ...
    try {
      fs.writeFileSync('./data/team/afl_fantasy_team.json', JSON.stringify(team, null, 2));
    } catch (saveErr) {
...
teamApi.get("/data", (_, res) => {
  try {
    ...
    let teamData;
    try {
      const fileData = fs.readFileSync('./data/team/afl_fantasy_team.json', 'utf8');
...
// Get lineup data - enriched team data with master stats for lineup page
teamApi.get("/lineup", async (req, res) => {
  console.log('[lineup] ENDPOINT HIT - Starting lineup request');
  try {
    // Load master stats for enrichment
    const masterStatsPath = path.join(process.cwd(), 'public/server/data/master_player_stats.json');
    console.log('[lineup] Master stats path:', masterStatsPath);
    let masterStats: any = {};
    
    if (fs.existsSync(masterStatsPath)) {
      console.log('[lineup] Master stats file exists');
      const masterStatsData = fs.readFileSync(masterStatsPath, 'utf8');
      masterStats = JSON.parse(masterStatsData);
    } else {
      console.log('[lineup] Master stats file not found');
    }
    ...
    const userTeamPath = path.join(process.cwd(), 'data/team/afl_fantasy_team.json');
    console.log('[lineup] Team data path:', userTeamPath);
    console.log('[lineup] File exists:', fs.existsSync(userTeamPath));
    
    if (fs.existsSync(userTeamPath)) {
      console.log('[lineup] Reading team data file...');
      const rawTeamData = fs.readFileSync(userTeamPath, 'utf8');
      const fullTeamData = JSON.parse(rawTeamData);
      
      // Extract currentRound data (the file structure has team data inside currentRound)
      if (fullTeamData.currentRound) {
        teamData = fullTeamData.currentRound;
        console.log('[lineup] Using currentRound data');
      } else {
        // Fallback: use the data directly if no currentRound structure
        teamData = fullTeamData;
        console.log('[lineup] Using direct data structure');
      }
    } else {
      // File doesn't exist, return 404
      console.error('[lineup] Fantasy team data file not found at:', userTeamPath);
    ...
    // Enrich team data with master stats
    const enrichPlayerData = (player: any, position: string) => {
      // Find player in master stats
      const masterPlayer = masterStats.players?.find((p: any) => 
        p.name?.toLowerCase().trim() === player.name?.toLowerCase().trim()
      );

      if (masterPlayer) {
        return {
          id: Math.abs(masterPlayer.name.split('').reduce((a, b) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0)),
          name: masterPlayer.name,
          team: masterPlayer.team,
          position: position,
          price: masterPlayer.price || player.price || 0,
          breakEven: masterPlayer.break_even || player.breakEven || 0,
          lastScore: masterPlayer.last_score || player.lastScore || 0,
          averagePoints: masterPlayer.average_points || player.averagePoints || 0,
          l3Average: masterPlayer.last_3_average || player.l3Average || 0,
          roundsPlayed: masterPlayer.games_played || player.roundsPlayed || 0,
          projScore: masterPlayer.projected_score || player.projScore || 0,
          nextOpponent: masterPlayer.nextOpponent || player.nextOpponent || "",
          isCaptain: player.isCaptain || false,
          isOnBench: player.isOnBench || false,
          secondaryPositions: player.secondaryPositions || []
        };
      }

      // Fallback if not found in master stats
      return {
        id: Math.abs((player.name || 'unknown').split('').reduce((a, b) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0)),
    ...
    // Process main lineup
    const enrichedData = {
      midfielders: (teamData.midfielders || []).map((player: any) => enrichPlayerData(player, 'MID')),
      defenders: (teamData.defenders || []).map((player: any) => enrichPlayerData(player, 'DEF')),
      forwards: (teamData.forwards || []).map((player: any) => enrichPlayerData(player, 'FWD')),
      rucks: (teamData.rucks || []).map((player: any) => enrichPlayerData(player, 'RUCK'))
    };
    ...
    res.status(500).json({
      status: "error",
      message: "Failed to get lineup data",
      data: {
        midfielders: [],
        defenders: [],
        forwards: [],
...
teamApi.get("/stats", (_, res) => {
  try {
    const stats = {
      team_data_exists: fs.existsSync('./data/team/afl_fantasy_team.json'),
