...
function runTool(tool: string, res: express.Response) {
  ...
  if (!allowedTools.includes(tool)) {
    return res.status(400).json({ status: "error", message: "Invalid tool" });
  }
  
  const pythonCode = `from captain_tools import ${tool}; import json; print(json.dumps(${tool}()))`;
  execFile("python3", ["-c", pythonCode], (err, stdout) => {
