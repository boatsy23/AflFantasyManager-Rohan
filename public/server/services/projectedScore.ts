...
export class ProjectedScoreService {
    ...
    return {
      last3Average:
        last3Scores.length > 0
          ? Math.round(
              (last3Scores.reduce((a, b) => a + b, 0) / last3Scores.length) *
                10,
            ) / 10
          : 0,
      last5Average:
  ...
  private async getOpponentStats(playerId: number, opponent: string) {
    ...
    const recentVsOpponent = await db
      .select()
      .from(playerRoundScores)
      .where(
        and(
  ...
  private async getVenueStats(playerId: number, venue: string) {
    ...
    const venueData = await db
      ...
      .from(venueHistory)
      .where(
        and(eq(venueHistory.playerId, playerId), eq(venueHistory.venue, venue)),
      )
      .limit(1);
  ...
  private async getPlayerFixture(team: string, round: number) {
    const fixture = await db
      .select()
      .from(fixtures)
      .where(and(eq(fixtures.round, round), eq(fixtures.homeTeam, team)))
      .limit(1);
    ...
    const awayFixture = await db
      .select()
      .from(fixtures)
      .where(and(eq(fixtures.round, round), eq(fixtures.awayTeam, team)))
      .limit(1);
  ...
  private async calculateFactors(
    ...
    if (opponent) {
      ...
      if (defenseData.length > 0) {
        ...
        opponentDifficulty = Math.max(
          0,
          100 - (defenseData[0].rank - 1) * 5.56,
        );
      }
    }
    ...
    if (recentForm.last3Average > player.averagePoints + 10)
      recentFormStr = "Excellent";
    else if (recentForm.last3Average > player.averagePoints + 5)
      recentFormStr = "Good";
    else if (recentForm.last3Average < player.averagePoints - 10)
      recentFormStr = "Poor";
    else if (recentForm.last3Average < player.averagePoints - 5)
      recentFormStr = "Below Average";
   ...
   */
  async calculateBatchProjections(
    inputs: ProjectedScoreInput[],
  ): Promise<ProjectedScoreResult[]> {
