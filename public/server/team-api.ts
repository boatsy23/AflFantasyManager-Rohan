...
teamApi.post("/upload", async (req, res) => {
  ...
  try {
    ...
    try {
      fs.writeFileSync('./afl_fantasy_team.json', JSON.stringify(team, null, 2));
    } catch (saveErr) {
...
teamApi.get("/data", (_, res) => {
  try {
    ...
    let teamData;
    try {
      const fileData = fs.readFileSync('./afl_fantasy_team.json', 'utf8');
...
teamApi.get("/lineup", async (req, res) => {
  try {
    ...
    const masterStatsPath = './server/data/master_player_stats.json';
    let masterStats: any = {};
    ...
    const userTeamPath = './afl_fantasy_team.json';
...
teamApi.get("/stats", (_, res) => {
  try {
    const stats = {
      team_data_exists: fs.existsSync('./afl_fantasy_team.json'),
