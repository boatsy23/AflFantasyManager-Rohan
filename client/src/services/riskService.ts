...
export async function fetchInjuryRisk() {
  const response = await apiRequest('GET', '/api/risk/injury');
  const data = await response.json();
  if (data.success) {
    return data.data;
  }
  throw new Error("Failed to fetch injury risk data");
}
...
export async function fetchConsistencyScore() {
  const response = await apiRequest('GET', '/api/risk/consistency');
  const data = await response.json();
  if (data.success) {
    return data.data;
  }
