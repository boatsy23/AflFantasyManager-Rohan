...
/**
 * Fetches price prediction for a specific player using the integrated MasterDataService algorithm
 * @param playerName - The name of the player
 * @param rounds - Number of rounds to predict (default: 3)
 * @returns Promise with price prediction data
 */
export const fetchPricePrediction = async (playerName: string, rounds: number = 3) => {
  try {
    const response = await fetch(`/api/price-prediction/player/${encodeURIComponent(playerName)}?rounds=${rounds}`);
    return response.json();
  } catch (error) {
