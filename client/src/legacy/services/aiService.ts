...
export async function fetchAITrade() {
  const response = await apiRequest("GET", "/api/ai/trade-suggestions");
  const data = await response.json();
  if (data.success && Array.isArray(data.data)) {
    return data.data;
  }
  throw new Error("Failed to fetch AI trade suggestions");
}
...
export async function fetchAICaptain() {
  const response = await apiRequest("GET", "/api/ai/captain-advisor");
  const data = await response.json();
  if (data.success && Array.isArray(data.data)) {
    return data.data;
  }
...
export async function fetchTeamStructure() {
  const response = await apiRequest("GET", "/api/ai/team_structure_analyzer");
  const data = await response.json();
