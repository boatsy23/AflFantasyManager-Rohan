...
export const fetchByeOptimizer = async () => {
  try {
    const response = await fetch('/api/context-tools/bye-optimizer');
    const data = await response.json();
    if (data.success) {
      return data.data;
    }
    throw new Error("Failed to fetch bye optimizer data");
  } catch (error) {
...
export const fetchVenueBias = async () => {
  try {
    const response = await fetch('/api/context-tools/venue-bias');
    const data = await response.json();
    if (data.success) {
      return data.data;
    }
    throw new Error("Failed to fetch venue bias data");
  } catch (error) {
