...
import { getTeamGuernsey, getTeamColors } from "@/utils/team-utils";
import { useState, useEffect } from "react";
// import ScoreBreakdownModule from "./score-breakdown-module"; // Disabled until fixed
// Mock data removed - using real data from MasterDataService
import { normalizeTeam, displayTeamName } from "../../constants/teams";
// Note: Using local getPlayerPrimaryPosition function instead
...
export default function PlayerDetailModal({
  ...
  // Debug log for player data (only if missing key fields)
  ...
  // Fetch real matchup difficulty data using team and position
  const { data: matchupData, isLoading: matchupLoading } = useQuery<MatchupData>({
  ...
  // Fetch projected scores for upcoming rounds
      ...
      <DialogContent className="max-w-md max-h-[90vh] p-0 overflow-y-auto bg-gray-900 border-gray-700">
        ...
        {activeSection === "performance" && (
          <div className="px-4 pb-4">
            ...
            {/* Score Breakdown Module - Disabled until data format is fixed */}
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
              <div className="text-white text-center">
                <p className="text-sm text-gray-400">Score breakdown analysis coming soon</p>
              </div>
            </div>
          </div>
        )}
                ...
                {matchupLoading ? (
                  ...
                  <div className="space-y-2">
                    {matchupData.fixtures.map((fixture, index) => {
                      ...
                      // Convert numeric difficulty to visual category
                      console.log(`Fixture ${fixture.round} vs ${fixture.opponent}: difficulty=${fixture.difficulty}`);
                      let difficultyCategory = "MED";
                      let difficultyColor = "bg-yellow-500";
                      if (fixture.difficulty <= 3) {
                        difficultyCategory = "EASY";
                        difficultyColor = "bg-green-500";
                      } else if (fixture.difficulty >= 7) {
                        difficultyCategory = "HARD";
                        difficultyColor = "bg-red-500";
                      }
                      console.log(`Result: ${difficultyCategory} (${difficultyColor})`);
