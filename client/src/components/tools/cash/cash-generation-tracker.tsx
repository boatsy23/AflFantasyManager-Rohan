...
// Get upcoming opponents from fixture data
const getUpcomingOpponents = (player: any): string[] => {
  // Use actual fixture data if available
  if (player.fixtures && Array.isArray(player.fixtures)) {
    return player.fixtures.slice(0, 5).map((fixture: any) => fixture.opponent || fixture.against);
  }
  if (player.nextOpponents && Array.isArray(player.nextOpponents)) {
    return player.nextOpponents.slice(0, 5);
  }
  // Return empty array if no fixture data available
  return [];
};
...
const transformPlayerData = (apiPlayers: any[], dvpData?: any): CashGenPlayer[] => {
  return apiPlayers.map((player, index) => {
    ...
    if (dvpData && player.team) {
      ...
      if (positionData) {
        ...
        if (teamData?.fixtures) {
          ...
          upcomingOpponents = dvpFixtures.map((f: any) => f.opponent);
        }
      }
    }
    ...
    return {
      ...
      upcomingOpponents: upcomingOpponents.length > 0 ? upcomingOpponents : getUpcomingOpponents(player),
      ...
      dvpDifficulty: dvpFixtures.map((f: any) => f.difficulty)
    };
  });
};
