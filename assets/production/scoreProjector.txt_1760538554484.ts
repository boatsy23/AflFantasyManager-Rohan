...
export class ScoreProjector {
  ...
  private async getPlayerByName(playerName: string): Promise<PlayerData | null> {
    ...
    // Calculate true season average from individual round scores (excluding 0s for bye/injury)
    let calculatedSeasonAverage = masterData.averagePoints; // fallback
    
    if (masterData.recentGameLogs && masterData.recentGameLogs.length > 0) {
      // Get all scores from the season, excluding 0s (bye/injury)
      const seasonScores = masterData.recentGameLogs
        .map((log: any) => log.FP)
        .filter((score: number) => score > 0); // Exclude 0s as requested
      
      if (seasonScores.length > 0) {
        const totalPoints = seasonScores.reduce((sum: number, score: number) => sum + score, 0);
        calculatedSeasonAverage = Math.round(totalPoints / seasonScores.length);
        console.log(`${playerName}: ${seasonScores.length} games played, total: ${totalPoints}, average: ${calculatedSeasonAverage}`);
      }
    }
    ...
    return {
      ...
      averagePoints: calculatedSeasonAverage, // Now using calculated average
